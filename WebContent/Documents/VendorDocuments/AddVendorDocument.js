vendorDocumentController.directive('fileModel', ['$parse', function ($parse) {
	return {
        restrict: 'A',
...
vendorDocumentController.controller("VendorDocumentAddCtrl", ['$scope','$window', '$http', '$resource','$location', '$routeParams','$filter', '$cookieStore','$rootScope','myService', function ($scope, $window, $http, $resource,$location,  $routeParams, $filter,  $cookieStore, $rootScope, myService) {
	...
	$scope.document.vendorId = $cookieStore.get('vendorId');
...
//	$scope.roleId = $cookieStore.get("roleId");
    ...
    $scope.cancelBtnAction = function(){
        ...
        $scope.verifyBtn = false;
        $scope.commentsShow = false;
        $scope.getData('documentController/getVendorDocumentById.json', { customerId: $cookieStore.get('customerId'),companyId:$cookieStore.get('companyId'),vendorId:($cookieStore.get('vendorId') != null && $cookieStore.get('vendorId') != "") ? $cookieStore.get('vendorId')  : 0, vendorDocumentId:$routeParams.id }, 'documentList');
    	...
    	$http(req).then(function (response) {
    		if(type == 'buttonsAction'){
    			...
    			if(response.data != undefined){
	    			...
	    			$scope.documentList.vendorList = response.data.vendorList;
	    			$scope.documentList.documentsList = response.data.documentList;
	    			if(response.data.vendorLocation != undefined){
	    				$scope.locationName = response.data.vendorLocation[0].locationName;
	    			}
	                ...
	                if( response.data.vendorList != undefined && response.data.vendorList != "" && response.data.vendorList.length == 1 ){
		                $scope.document.vendorId = response.data.vendorList[0].id;	
		                $scope.vendorName = response.data.vendorList[0].name
		            }
	...
	$scope.save =function($event){
    	...
    	if($('#documentForm').valid()){
		 ...
		 //   $scope.getData('invoiceController/saveVendorDocument.json', angular.toJson($scope.document), 'saveDocument',angular.element($event.currentTarget)); 
	 		...
	 		$http.post('documentController/saveVendorDocument.json', formData, {
 				...
 				if(response.id > 0){
	            	...
	            	$scope.Messager('success', 'success', 'Vendor Document Saved Successfully');
	            	$scope.document.documentDate =  $filter('date')( $('#documentDate').val(), 'dd/MM/yyyy');
	            	$location.path('/addVendorDocument/'+response.id);
